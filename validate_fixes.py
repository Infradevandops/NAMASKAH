...
class SecurityValidator:
    ...
    def test_server_health(self):
        ...
        if response.get("status_code") == 200:
            ...
            self.log_result(
                "server_health",
                False,
                f"Server not responding: {response.get('error', 'Unknown error')}",
            )
    ...
    def test_security_headers(self):
        ...
        if response.get("status_code") in [
            200,
            404,
        ]:  # Either works, we just need headers
            ...
            if not missing_headers:
                self.log_result(
                    "security_headers", True, "All security headers present"
                )
            else:
                self.log_result(
                    "security_headers",
            ...
            self.log_result(
                "security_headers",
                False,
                "Could not test headers - server not responding",
            )
    ...
    def test_rate_limiting(self):
        ...
        if rate_limited:
            self.log_result(
                "rate_limiting",
                True,
                f"Rate limiting triggered after {request_count} requests",
            )
        else:
            self.log_result(
                "rate_limiting",
    ...
    def test_input_sanitization(self):
        ...
        for malicious_input in malicious_inputs:
            response = self.make_request(
                "/verify/create", "POST", {"service_name": malicious_input}
            )
        ...
        if sanitized_count >= len(malicious_inputs) * 0.8:  # 80% threshold
            self.log_result(
                "input_sanitization",
                True,
                f"Input sanitization working ({sanitized_count}/{len(malicious_inputs)} blocked)",
            )
        else:
            self.log_result(
                "input_sanitization",
                False,
                f"Input sanitization insufficient ({sanitized_count}/{len(malicious_inputs)} blocked)",
            )
    ...
    def test_bulk_verification(self):
        ...
        response = self.make_request(
            "/verify/bulk", "POST", {"services": ["telegram", "whatsapp"]}
        )
        ...
        if response.get("status_code") in [401, 422]:
            self.log_result(
                "bulk_verification",
                True,
                "Bulk verification endpoint exists and requires auth",
            )
        elif response.get("status_code") == 404:
            self.log_result(
                "bulk_verification", False, "Bulk verification endpoint not found"
            )
        else:
            self.log_result(
                "bulk_verification", True, "Bulk verification endpoint responding"
            )
        ...
        try:
            ...
            if response.get("status_code") in [426, 400, 404]:
                if response.get("status_code") == 404:
                    self.log_result(
                        "websocket_endpoint", False, "WebSocket endpoint not found"
                    )
                else:
                    self.log_result(
                        "websocket_endpoint", True, "WebSocket endpoint exists"
                    )
            else:
                self.log_result(
                    "websocket_endpoint", True, "WebSocket endpoint responding"
                )

        except Exception as e:
            self.log_result(
                "websocket_endpoint", False, f"WebSocket test failed: {str(e)}"
            )
    ...
    def test_api_documentation(self):
        ...
        if response.get("status_code") == 200:
            ...
            self.log_result(
                "api_documentation", False, "API documentation not accessible"
            )
    ...
    def test_admin_security(self):
        ...
        if response.get("status_code") in [401, 403]:
            self.log_result(
                "admin_security", True, "Admin endpoints properly protected"
            )
        elif response.get("status_code") == 404:
            self.log_result("admin_security", False, "Admin endpoints not found")
        else:
            self.log_result(
                "admin_security", False, "Admin endpoints not properly protected"
            )
