...
class GradualRolloutValidator:
    ...
    async def validate_rollout_stage(
        self, stage: RolloutStage, current_metrics: Dict[str, float], sample_size: int
    ) -> ValidationResult:
        ...
        if (
            "response_time" in current_metrics
            and "response_time" in self.baseline_metrics
        ):
            ...
            if (
                response_time_increase
                > self.validation_thresholds["response_time_increase"]
            ):
                issues.append(
                    f"Response time increased by {response_time_increase:.2f}%"
                )

        # Validate conversion rate
        if (
            "conversion_rate" in current_metrics
            and "conversion_rate" in self.baseline_metrics
        ):
            conversion_decrease = (
                self.baseline_metrics["conversion_rate"]
                - current_metrics["conversion_rate"]
            )
            if (
                conversion_decrease
                > self.validation_thresholds["conversion_rate_decrease"]
            ):
                issues.append(
                    f"Conversion rate decreased by {conversion_decrease:.2f}%"
                )

        passed = len(issues) == 0

        return ValidationResult(
            stage=stage, passed=passed, metrics=current_metrics, issues=issues
        )
    ...
    async def run_ab_test_validation(
        ...
        conversion_diff = (
            treatment_metrics["conversion_rate"] - control_metrics["conversion_rate"]
        )
        error_rate_diff = (
            treatment_metrics["error_rate"] - control_metrics["error_rate"]
        )
        response_time_diff = (
            treatment_metrics["response_time"] - control_metrics["response_time"]
        )

        # Determine winner
        winner = "treatment"
        if (
            conversion_diff < -2.0  # Conversion rate dropped significantly
            or error_rate_diff > 2.0  # Error rate increased significantly
            or response_time_diff > 500  # Response time increased by 500ms
        ):
            winner = "control"
...
class BusinessMetricsTracker:
    ...
    def analyze_trends(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "overall_health": (
                "good"
                if all(t.get("trend") != "declining" for t in trends.values())
                else "concerning"
            ),
        }
...
class AutomaticRollbackTrigger:
    ...
    async def _check_rollback_conditions(self) -> tuple[bool, str]:
        ...
        if (
            current_metrics["response_time"]
            > self.rollback_conditions["response_time_spike"]
        ):
            return (
                True,
                f"Response time spike: {current_metrics['response_time']:.2f}ms",
            )

        if (
            current_metrics["user_complaints"]
            >= self.rollback_conditions["user_complaints"]
        ):
            return (
                True,
                f"User complaints threshold reached: {current_metrics['user_complaints']}",
            )
...
async def run_gradual_rollout_validation():
    ...
    baseline = {"error_rate": 1.5, "response_time": 200.0, "conversion_rate": 85.0}
    ...
    stages = [
        (
            RolloutStage.CANARY,
            {"error_rate": 1.8, "response_time": 210.0, "conversion_rate": 84.5},
        ),
        (
            RolloutStage.STAGE_10,
            {"error_rate": 2.0, "response_time": 220.0, "conversion_rate": 84.0},
        ),
        (
            RolloutStage.STAGE_50,
            {"error_rate": 2.2, "response_time": 230.0, "conversion_rate": 83.5},
        ),
        (
            RolloutStage.FULL,
            {"error_rate": 2.5, "response_time": 240.0, "conversion_rate": 83.0},
        ),
    ]
    ...
    treatment_metrics = {
        "conversion_rate": 86.0,
        "error_rate": 1.8,
        "response_time": 210.0,
    }

    ab_result = await validator.run_ab_test_validation(
