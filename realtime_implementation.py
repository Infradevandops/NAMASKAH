...
class WebSocketManager:
    ...
    async def connect(self, websocket: WebSocket, user_id: str, verification_id: str = None) -> str:
        ...
        if connection_id in self.active_connections:
            ...
            if connection.verification_id:
                if connection.verification_id in self.verification_subscribers:
                    self.verification_subscribers[connection.verification_id].discard(connection_id)
    ...
    async def broadcast_to_verification(self, verification_id: str, message: Dict[str, Any]):
        ...
        logging.info(f"Broadcast to verification {verification_id}: {successful_sends}/{len(subscribers)} successful")
    ...
    def get_connection_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "connections_by_age": {
                "under_1min": len([c for c in self.active_connections.values() 
                                 if (now - c.connected_at).total_seconds() < 60]),
                "1_to_5min": len([c for c in self.active_connections.values() 
                                if 60 <= (now - c.connected_at).total_seconds() < 300]),
                "over_5min": len([c for c in self.active_connections.values() 
        ...
        try:
            while time.time() - start_time < self.max_monitoring_time:
                try:
                        ...
                        if any(msg.get("code") for msg in new_messages):
                            await self.websocket_manager.broadcast_to_verification(
                                verification_id,
                                {
                                    ...
                                    "verification_id": verification_id,
                                    "code": next(msg["code"] for msg in new_messages if msg.get("code")),
                                    "timestamp": datetime.utcnow().isoformat()
                                }
                            )
                            break
                    ...
                    logging.error(f"Error monitoring verification {verification_id}: {str(e)}")
...
class NotificationManager:
    def __init__(self, websocket_manager: WebSocketManager, cache_manager: CacheManager):
        self.websocket_manager = websocket_manager
    ...
    async def send_notification(self, user_id: str, notification: Dict[str, Any]):
        ...
        # Try WebSocket first
        user_connections = [
            conn_id for conn_id, conn in self.websocket_manager.active_connections.items()
            if conn.user_id == user_id
        ]
        
        websocket_sent = False
        for conn_id in user_connections:
            if await self.websocket_manager.send_personal_message(conn_id, {
                "type": "notification",
                **notification
            }):
                websocket_sent = True
        ...
        if not websocket_sent:
            await self.cache_manager.set(f"notifications:{user_id}", notification, ttl=3600)
