...
async def create_verification(
    ...
    if user.free_verifications > 0:
        ...
        raise InsufficientCreditsError(
            f"Insufficient credits. Need {base_cost}, have {user.credits}"
        )
...
async def get_verification_status(verification_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        new_status = (
            "completed"
            if details.get("state") == "verificationCompleted"
            else "pending"
        )
...
async def retry_verification(
    ...
    try:
        if retry_data.retry_type == "voice":
            ...
            details = await textverified_service.get_verification_status(
                new_verification_id
            )
...
@router.post(
    "/rentals", response_model=NumberRentalResponse, status_code=status.HTTP_201_CREATED
)
async def create_number_rental(
    ...
    if user.credits < total_cost:
        raise InsufficientCreditsError(
            f"Insufficient credits. Need {total_cost}, have {user.credits}"
        )
...
def extend_rental(
    ...
    if user.credits < extension_cost:
        raise InsufficientCreditsError(
            f"Insufficient credits. Need {extension_cost}, have {user.credits}"
        )
