...
def get_user_analytics(
    ...
    for i in range(period):
        ...
        daily_usage.append(
            {"date": day_start.strftime("%Y-%m-%d"), "count": day_verifications}
        )
...
def get_cost_analysis(
    ...
    for i in range(6):  # Last 6 months
        ...
        monthly_spending.append(
            {"month": month_start.strftime("%Y-%m"), "amount": abs(month_spent)}
        )
...
def export_data(
    user_id: str = Depends(get_current_user_id),
    data_type: str = Query(
        "verifications", description="Data type: verifications or transactions"
    ),
