...
from models import (
    ...
    ConversationFilters, MessageFilters, ConversationStatus, MessageType
)
...
async def search_messages(
    ...
    try:
        ...
        if created_after:
            try:
                parsed_after = datetime.fromisoformat(created_after.replace('Z', '+00:00'))
            except ValueError:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
        ...
        if created_before:
            try:
                parsed_before = datetime.fromisoformat(created_before.replace('Z', '+00:00'))
            except ValueError:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
