...
def validate_service_name(service_name: str) -> str:
    ...
    allowed_services = [
        'telegram', 'whatsapp', 'discord', 'google', 'instagram', 'facebook', 
        'twitter', 'tiktok', 'snapchat', 'linkedin', 'uber', 'airbnb', 'netflix',
        'spotify', 'amazon', 'microsoft', 'apple', 'yahoo', 'outlook', 'gmail',
        'signal', 'viber', 'line', 'wechat', 'skype', 'zoom', 'slack', 'reddit',
        'pinterest', 'tumblr', 'twitch', 'steam', 'epic', 'paypal', 'coinbase',
        'binance', 'kraken', 'robinhood', 'cashapp', 'venmo', 'zelle'
    ]
...
def validate_verification_request(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if carrier:
        ...
        if carrier not in allowed_carriers:
            raise ValidationError(f"Invalid carrier. Must be one of: {', '.join(allowed_carriers)}")
...
def add_security_headers(response):
    ...
    response.headers["Content-Security-Policy"] = "default-src 'self'; script-src 'self' 'unsafe-inline' https://accounts.google.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; connect-src 'self' wss: https:;"
    return response
