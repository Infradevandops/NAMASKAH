...
class ConfigValidator:
    ...
    def validate_all(self) -> bool:
        ...
        try:
            ...
            print(f"✅ Configuration loaded for environment: {self.settings.environment}")
    ...
    def _validate_external_apis(self):
        ...
        if self.settings.environment == 'production':
            if self.settings.paystack_secret_key and not self.settings.paystack_secret_key.startswith('sk_live_'):
                self.errors.append("Production requires live Paystack secret key (sk_live_)")
    ...
    def _validate_ssl_config(self):
        ...
        if self.settings.environment == 'production':
            if self.settings.ssl_cert_path and not os.path.exists(self.settings.ssl_cert_path):
                self.errors.append(f"SSL certificate file not found: {self.settings.ssl_cert_path}")
            
            if self.settings.ssl_key_path and not os.path.exists(self.settings.ssl_key_path):
                self.errors.append(f"SSL private key file not found: {self.settings.ssl_key_path}")
