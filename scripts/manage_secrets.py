...
def rotate_secret(key: str):
    ...
    if key not in ["SECRET_KEY", "JWT_SECRET_KEY"]:
        print(f"❌ Cannot auto-rotate {key}. Only SECRET_KEY and JWT_SECRET_KEY are supported.")
        return
...
def main():
    ...
    gen_parser.add_argument("--env", required=True, choices=["development", "staging", "production"],
