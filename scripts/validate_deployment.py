...
class DeploymentValidator:
    ...
    async def _validate_docker_services(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                failed_services = [s for s in critical_services if not services.get(s, False)]
    ...
    async def _validate_health_endpoints(self):
        ...
        try:
            ...
            if len(successful_endpoints) == len(endpoints):
                ...
                failed_endpoints = [ep for ep in endpoints if ep not in successful_endpoints]
                self.results.append(ValidationResult(
                    "Health Endpoints",
                    "fail",
    ...
    async def _validate_api_endpoints(self):
        ...
        try:
            ...
            if len(successful_endpoints) >= len(public_endpoints) * 0.8:  # 80% success rate
                self.results.append(ValidationResult(
                    "API Endpoints",
                    "pass",
            ...
            if response_times:
                ...
                if avg_time < 500:  # Less than 500ms average
                    self.results.append(ValidationResult(
                        ...
                        {"avg_ms": avg_time, "max_ms": max_time, "samples": len(response_times)},
                        (time.time() - start_time) * 1000
                    ))
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if success_rate > 0.9:  # 90% success rate
                    self.results.append(ValidationResult(
                        ...
                        {"success_rate": success_rate, "total_requests": len(responses)},
                        (time.time() - start_time) * 1000
                    ))
