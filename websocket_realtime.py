...
async def handle_websocket_message(
    websocket: WebSocket, user_id: str, data: dict, db: Session
):
    ...
    if message_type == "ping":
        ...
        if verification_id:
            ...
            await websocket.send_text(
                json.dumps({"type": "subscribed", "verification_id": verification_id})
            )
        ...
        if verification_id:
            await websocket.send_text(
                json.dumps({"type": "unsubscribed", "verification_id": verification_id})
            )
...
async def notify_verification_update(
    verification_id: str, user_id: str, status: str, phone_number: str = None
):
...
async def notify_sms_received(
    verification_id: str, user_id: str, sms_content: str, sender: str = None
):
...
async def notify_low_balance(
    user_id: str, current_balance: float, threshold: float = 1.0
):
...
async def notify_payment_received(
    user_id: str, amount: float, new_balance: float, reference: str
):
...
async def notify_rental_expiring(
    user_id: str, rental_id: str, phone_number: str, expires_in_minutes: int
):
...
def add_websocket_routes(app):
                ...
                while True:
                    ...
                    try:
                        ...
                        await handle_websocket_message(
                            websocket, user_id, message_data, None
                        )
    ...
    async def verification_websocket(
        websocket: WebSocket, verification_id: str, token: str = None
    ):
