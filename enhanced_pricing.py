...
class EnhancedPricingEngine:
    ...
    def calculate_demand_multiplier(
        self, service_name: str, current_hour: int
    ) -> float:
    ...
    def calculate_success_rate_modifier(
        self, service_name: str, success_rate: float
    ) -> float:
    ...
    def calculate_carrier_premium(
        self, carrier_preference: Optional[str]
    ) -> Tuple[float, str]:
    ...
    def calculate_dynamic_price(
        ...
        factors.demand_multiplier = self.calculate_demand_multiplier(
            service_name, current_time.hour
        )
        factors.success_rate_modifier = self.calculate_success_rate_modifier(
            service_name, success_rate
        )
        ...
        carrier_premium, carrier_reason = self.calculate_carrier_premium(
            carrier_preference
        )
        ...
        # Apply plan discount
        plan_discounts = {"starter": 0.0, "pro": 0.15, "turbo": 0.25}
        ...
        if factors.volume_discount > 0:
            discounts_applied.append(
                f"Volume discount: {factors.volume_discount*100:.0f}%"
            )
        ...
        if time_reason in ["peak_hours", "weekend_surcharge"]:
            ...
            surcharges_applied.append(
                f"{time_reason.replace('_', ' ').title()}: +{surcharge_pct:.0f}%"
            )

        if factors.carrier_premium > 0:
            surcharges_applied.append(
                f"Carrier premium: +${factors.carrier_premium:.2f}"
            )

        if factors.priority_surcharge > 0:
            surcharges_applied.append(
                f"Priority processing: +${factors.priority_surcharge:.2f}"
            )
    ...
    def get_pricing_forecast(
        self, service_name: str, hours_ahead: int = 24
    ) -> List[Dict]:
        ...
        for hour in range(hours_ahead):
            ...
            result = self.calculate_dynamic_price(
                service_name=service_name, current_time=future_time
            )
            ...
            forecast.append(
                {
                    ...
                    "factors": {
                        "demand": (
                            ...
                            > self.base_tiers[self.get_service_tier(service_name)][
                                "base_price"
                            ]
                            else "normal"
                        )
    ...
    def optimize_timing_for_cost(
        ...
        return {
            ...
            "savings_percentage": (
                round((savings / current["price"]) * 100, 1)
                if current["price"] > 0
                else 0
            ),
...
class SubscriptionPricingManager:
    ...
    def calculate_plan_value(
        self, monthly_usage: int, avg_price_per_verification: float = 1.0
    ) -> Dict:
        ...
        for plan_id, plan in self.plans.items():
            ...
            results[plan_id] = {
                ...
                "cost_per_verification": (
                    round(total_monthly_cost / monthly_usage, 2)
                    if monthly_usage > 0
                    else 0
                ),
                "break_even_usage": self._calculate_break_even(
                    plan, avg_price_per_verification
                ),
                "recommended": savings > 0
                and monthly_usage
                >= plan.get("limits", {}).get("monthly_verifications", 0),
            }
    ...
    def recommend_plan(self, monthly_usage: int, avg_price: float = 1.0) -> Dict:
        ...
        best_plan = min(plan_values.items(), key=lambda x: x[1]["total_monthly_cost"])
        ...
        return {
            ...
            "reasoning": self._get_recommendation_reasoning(
                best_plan[0], monthly_usage, plan_values
            ),
        }

    def _get_recommendation_reasoning(
        self, plan_id: str, usage: int, all_plans: Dict
    ) -> str:
        ...
        if plan_id == "starter":
            return (
                f"Starter plan is most cost-effective for {usage} verifications/month"
            )
