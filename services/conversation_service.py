...
from models import (
    User, Conversation, Message, conversation_participants,
    ConversationStatus, MessageType, ConversationFilters, MessageFilters,
...
class ConversationService:
    ...
    async def add_participants(
        ...
        try:
            ...
            new_participants = self.db.query(User).filter(
    ...
    async def remove_participant(
        ...
        try:
            ...
            logger.info(f"Participant {participant_id} removed from conversation {conversation_id}")
    ...
    async def send_message(
        ...
        try:
            ...
            # Handle SMS messages
            if message_data.message_type in [MessageType.SMS_OUTBOUND, MessageType.SMS_INBOUND]:
            ...
            logger.info(f"Message sent to conversation {conversation_id} by user {sender_id}")
    ...
    async def search_messages(
        ...
        try:
            ...
            user_conversations = self.db.query(Conversation.id).join(
                conversation_participants
            ).filter(
                conversation_participants.c.user_id == user_id
            ).subquery()
            ...
            search_query = self.db.query(Message).options(
                joinedload(Message.sender),
            ...
            if filters:
                ...
                if filters.sender_id:
                    search_query = search_query.filter(Message.sender_id == filters.sender_id)
                if filters.is_read is not None:
                    search_query = search_query.filter(Message.is_read == filters.is_read)
                if filters.created_after:
                    search_query = search_query.filter(Message.created_at >= filters.created_after)
                if filters.created_before:
                    search_query = search_query.filter(Message.created_at <= filters.created_before)
                if hasattr(filters, 'conversation_id') and filters.conversation_id:
                    search_query = search_query.filter(Message.conversation_id == filters.conversation_id)
    ...
    async def search_conversations(
        ...
        try:
            ...
            search_query = self.db.query(Conversation).options(
                joinedload(Conversation.participants),
                joinedload(Conversation.creator)
            ).join(
                conversation_participants
            ...
            if query:
                ...
                participant_search = self.db.query(conversation_participants.c.conversation_id).join(
                    User, conversation_participants.c.user_id == User.id
                ).filter(
                    or_(*[User.username.ilike(f"%{word}%") for word in words if len(word) >= 2])
                ).subquery()
    ...
    async def get_user_mentions(
        ...
        try:
            ...
            if conversation_id:
                ...
                conversation = await self.get_conversation(conversation_id, user_id)
                if conversation:
                    participant_ids = [p.id for p in conversation.participants if p.id != user_id]
                    if participant_ids:
    ...
    async def mark_messages_as_read(
        ...
        try:
            ...
            if up_to_message_id:
                up_to_message = self.db.query(Message).filter(
