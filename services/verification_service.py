#!/usr/bin/env python3
"""
Enhanced Verification Service for SMSPROJ Communication Platform
...
from models.user_models import User, VerificationRequest
...
class CodeExtractionService:
    ...
    @classmethod
    def extract_verification_codes(cls, message_content: str, service_name: str = None) -> List[str]:
        """
    ...
    @classmethod
    def identify_service_patterns(cls, message_content: str, service_name: str) -> Dict[str, Any]:
        """
...
class VerificationService:
    ...
    def __init__(self, db_session: Session, textverified_client: TextVerifiedClient = None):
        self.db = db_session
        self.textverified_client = textverified_client
        self.code_extractor = CodeExtractionService()
        self.notification_service = NotificationService()
        
    async def create_verification(self, user_id: str, service_name: str, capability: str = "sms") -> VerificationRequest:
        """
        ...
        try:
            ...
            logger.info(f"Created verification {verification.id} for user {user_id}, service {service_name}")
    ...
    async def get_verification_number(self, user_id: str, verification_id: str) -> str:
        ...
        try:
            ...
            if not verification.phone_number:
                ...
                logger.info(f"Retrieved phone number {phone_number} for verification {verification_id}")
    ...
    async def check_verification_messages(self, user_id: str, verification_id: str) -> List[str]:
        """
        ...
        try:
            ...
            if messages:
                ...
                if all_codes and not verification.verification_code:
                    verification.verification_code = all_codes[0]  # Use first code found
                    ...
                    await self.notification_service.send_verification_completed(
                        user_id, verification.service_name, verification.verification_code
                    )
                    
                    logger.info(f"Verification {verification_id} completed with code {verification.verification_code}")
    ...
    async def get_verification_history(self, user_id: str, filters: Dict[str, Any] = None) -> List[VerificationRequest]:
        """
        ...
        try:
            ...
            if filters:
                if 'service_name' in filters:
                    query = query.filter(
                        VerificationRequest.service_name.ilike(f"%{filters['service_name']}%")
                    )
                
                if 'status' in filters:
                    query = query.filter(VerificationRequest.status == filters['status'])
                
                if 'date_from' in filters:
                    query = query.filter(VerificationRequest.created_at >= filters['date_from'])
                
                if 'date_to' in filters:
                    query = query.filter(VerificationRequest.created_at <= filters['date_to'])
            
            # Order by creation date (newest first)
            verifications = query.order_by(desc(VerificationRequest.created_at)).all()
            
            logger.info(f"Retrieved {len(verifications)} verification records for user {user_id}")
    ...
    async def search_verifications(self, user_id: str, search_query: str, filters: Dict[str, Any] = None) -> List[VerificationRequest]:
        """
        ...
        try:
            ...
            logger.info(f"Search '{search_query}' returned {len(results)} results for user {user_id}")
    ...
    async def cancel_verification(self, user_id: str, verification_id: str) -> bool:
        ...
        try:
            ...
            if verification.status in ['completed', 'cancelled', 'expired']:
                raise ValueError(f"Cannot cancel verification with status: {verification.status}")
            
            # Cancel with TextVerified
            await self.textverified_client.cancel_verification(verification.textverified_id)
    ...
    async def get_verification_statistics(self, user_id: str, period_days: int = 30) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            ).group_by(VerificationRequest.status).all()
            ...
            ).group_by(VerificationRequest.service_name).all()
            
            # Calculate success rate
            total_verifications = sum(count for _, count in status_counts)
            completed_verifications = next((count for status, count in status_counts if status == 'completed'), 0)
    ...
    async def export_verification_data(self, user_id: str, format_type: str = "json", filters: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(f"Exported {len(export_data)} verification records for user {user_id} in {format_type} format")
    ...
    async def _get_user_verification(self, user_id: str, verification_id: str) -> VerificationRequest:
        """Get verification request ensuring it belongs to the user"""
        verification = self.db.query(VerificationRequest).filter(
            and_(
        ...
        if not verification:
            raise ValueError(f"Verification {verification_id} not found for user {user_id}")
    ...
    async def _check_verification_limits(self, user: User):
        ...
        daily_count = self.db.query(VerificationRequest).filter(
            and_(
        ...
        if daily_count >= user_limit:
            raise ValueError(f"Daily verification limit ({user_limit}) reached for subscription plan: {user.subscription_plan}")
    ...
    async def _monitor_verification(self, verification_id: str):
        ...
        try:
            ...
            while attempt < max_attempts:
                ...
                if not verification or verification.status in ['completed', 'cancelled', 'expired']:
                    break
                ...
                try:
                    ...
                    if messages and not verification.verification_code:
                        ...
                        if all_codes:
                            ...
                            # Send notification
                            await self.notification_service.send_verification_completed(
                                verification.user_id, verification.service_name, verification.verification_code
                            )
                            
                            logger.info(f"Auto-completed verification {verification_id} with code {verification.verification_code}")
                            break
                
                except Exception as e:
                    logger.warning(f"Error monitoring verification {verification_id}: {e}")
            ...
            if attempt >= max_attempts:
                ...
                if verification and verification.status == 'pending':
                    ...
                    logger.info(f"Verification {verification_id} expired after monitoring timeout")
