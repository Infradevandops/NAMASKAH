...
from models.database import (
    User, PhoneNumber, Conversation, ConversationParticipant, 
    Message, Contact, MessageType, MessageStatus, NumberStatus
)
...
class MessagingService:
    ...
    def __init__(self, db_session: Session, sms_client=None):
        self.db = db_session
        self.sms_client = sms_client or MockTwilioClient()
        self.active_connections: Dict[str, Any] = {}  # WebSocket connections
    
    async def send_internal_message(self, sender_id: str, recipient_id: str, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            await self._notify_users([recipient_id], {
                "type": "new_message",
    ...
    async def send_sms_to_external(self, sender_id: str, phone_number: str, content: str, 
        ...
        try:
            ...
            if from_number_id:
                from_number = self.db.query(PhoneNumber).filter(
                    and_(PhoneNumber.id == from_number_id, PhoneNumber.owner_id == sender_id)
                ).first()
            else:
                # Use first available number
                from_number = self.db.query(PhoneNumber).filter(
                    and_(PhoneNumber.owner_id == sender_id, PhoneNumber.status == NumberStatus.ACTIVE)
                ).first()
            ...
            conversation = self._get_or_create_external_conversation(sender_id, phone_number)
            ...
            # Update usage
            self._update_usage(sender_id, from_number.id, sms_sent=1, cost=from_number.sms_cost)
            
            self.db.commit()
            
            # Real-time update for sender
            await self._notify_users([sender_id], {
                "type": "sms_sent",
                "message": self._serialize_message(message),
                "conversation_id": conversation.id
            })
    ...
    async def handle_incoming_sms(self, from_number: str, to_number: str, content: str, 
        ...
        try:
            ...
            await self._notify_users([phone_number.owner_id], {
                "type": "sms_received",
                "message": self._serialize_message(message),
                "conversation_id": conversation.id,
    ...
    def get_user_conversations(self, user_id: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        for conv in conversations:
            ...
            participants = self.db.query(User).join(ConversationParticipant).filter(
                and_(
            ...
            result.append({
                "id": conv.id,
                "name": conv.name or self._get_conversation_name(conv, participants),
    ...
    def get_user_contacts(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        for contact in contacts:
            ...
            if contact.is_platform_user and contact.platform_user_id:
                platform_user = self.db.query(User).filter(User.id == contact.platform_user_id).first()
                if platform_user:
    ...
    def _get_or_create_conversation(self, user_ids: List[str]) -> Conversation:
        ...
        if len(user_ids) == 2:
            ...
            existing = self.db.query(Conversation).join(ConversationParticipant).filter(
                ...
                self.db.query(ConversationParticipant.user_id).filter(
                    ConversationParticipant.conversation_id == Conversation.id
                ).filter(ConversationParticipant.user_id.in_(user_ids)).count() == len(user_ids)
            ).first()
    ...
    def _get_or_create_external_conversation(self, user_id: str, external_number: str) -> Conversation:
        """Get or create conversation with external number"""
        existing = self.db.query(Conversation).filter(
            and_(
                Conversation.external_number == external_number,
                Conversation.participants.any(ConversationParticipant.user_id == user_id)
            )
        ).first()
    ...
    def _update_usage(self, user_id: str, phone_number_id: str, sms_sent: int = 0, 
                     sms_received: int = 0, cost: float = 0.0):
    ...
    def _get_conversation_name(self, conversation: Conversation, participants: List[User]) -> str:
