...
class AdvancedAnalytics:
    ...
    def _get_cached_or_compute(self, cache_key: str, compute_func, ttl_minutes: int = 30):
    ...
    def get_real_time_metrics(self) -> Dict:
        ...
        # Real-time counters
        active_verifications = self.db.query(Verification).filter(
            Verification.status == "pending"
        ).count()
        
        hourly_verifications = self.db.query(Verification).filter(
            Verification.created_at >= hour_ago
        ).count()
        
        hourly_completions = self.db.query(Verification).filter(
            and_(
        ...
        hourly_success_rate = (hourly_completions / hourly_verifications * 100) if hourly_verifications > 0 else 0
        
        # Revenue (last 24 hours)
        daily_revenue = self.db.query(func.sum(Transaction.amount)).filter(
            and_(
        ...
        active_users = self.db.query(Verification.user_id).filter(
    ...
    def _calculate_system_health(self) -> str:
        ...
        recent_verifications = self.db.query(Verification).filter(
            Verification.created_at >= hour_ago
        ).all()
        ...
        success_count = len([v for v in recent_verifications if v.status == "completed"])
    ...
    def get_service_performance_analysis(self, days: int = 30) -> Dict:
        ...
        def compute():
            ...
            service_metrics = defaultdict(lambda: {
                ...
                'delivery_times': [], 'revenue': 0, 'hours': defaultdict(int)
            })
            ...
            for service, data in service_metrics.items():
                ...
                avg_delivery = statistics.mean(data['delivery_times']) if data['delivery_times'] else 0
                
                # Find peak hours
                peak_hours = sorted(data['hours'].items(), key=lambda x: x[1], reverse=True)[:3]
                ...
                results[service] = PerformanceMetrics(
                    ...
                    user_satisfaction=self._calculate_satisfaction_score(success_rate, avg_delivery),
    ...
    def _calculate_satisfaction_score(self, success_rate: float, avg_delivery: float) -> float:
    ...
    def _analyze_failure_reasons(self, service_name: str, start_date: datetime) -> Dict[str, int]:
        """Analyze failure reasons for a service"""
        from main import Verification
        
        # This is a simplified version - in reality, you'd have more detailed failure tracking
        failed_verifications = self.db.query(Verification).filter(
            and_(
    ...
    def _predict_revenue_trends(self) -> Optional[PredictiveInsight]:
        ...
        if len(revenues) >= 7:
            ...
            previous_avg = statistics.mean(revenues[-14:-7]) if len(revenues) >= 14 else recent_avg
    ...
    def get_user_behavior_analysis(self) -> Dict:
        ...
        def compute():
            ...
            # User segmentation
            users_with_activity = self.db.query(User.id).join(Verification).filter(
            ...
            for user_id_tuple in users_with_activity:
                ...
                user_verifications = self.db.query(Verification).filter(
                    and_(
            ...
            return {
                ...
                'top_services': dict(sorted(service_preferences.items(), key=lambda x: x[1], reverse=True)[:10]),
                'peak_usage_hours': [hour for hour, count in peak_hours],
                'total_active_users': len(users_with_activity),
                'avg_verifications_per_user': statistics.mean([len(hours) for hours in usage_patterns.values()]) if usage_patterns else 0
            }
    ...
    def get_financial_analytics(self) -> Dict:
        ...
        def compute():
            ...
            # Revenue analysis
            revenue_data = self.db.query(Transaction).filter(
                and_(
                    Transaction.type == "debit",
                    Transaction.created_at >= thirty_days_ago
                )
            ...
            # Customer metrics
            paying_customers = self.db.query(Transaction.user_id).filter(
                and_(
            ...
            return {
                ...
                'arpu': round(arpu, 2),
                'daily_revenue_trend': {str(date): round(amount, 2) for date, amount in sorted(daily_revenue.items())},
    ...
    def _generate_recommendations(self, kpis: Dict, insights: List[PredictiveInsight]) -> List[str]:
        """Generate actionable recommendations"""
        recommendations = []
        
        # Revenue-based recommendations
        if kpis['revenue_growth'] < -10:
            recommendations.append("Revenue declining - consider promotional campaigns or new service offerings")
        elif kpis['revenue_growth'] > 20:
            recommendations.append("Strong revenue growth - consider scaling infrastructure to meet demand")
        
        # Success rate recommendations
        if kpis['success_rate'] < 90:
            recommendations.append("Success rate below target - investigate service provider issues")
        
        # System health recommendations
        if kpis['system_health'] in ['degraded', 'critical']:
            recommendations.append("System health issues detected - immediate attention required")
...
async def get_real_time_dashboard_data(db: Session) -> Dict:
    ...
    return {
        ...
        'insights': [asdict(insight) for insight in analytics.get_predictive_insights()[:3]],
