...
class EnhancedTextVerifiedClient:
    ...
    def _update_rate_limits(self, response: requests.Response):
        ...
        if reset_time:
            self.rate_limit_reset = datetime.fromtimestamp(int(reset_time), timezone.utc)
    ...
    async def get_available_services(self) -> List[Dict]:
        ...
        try:
            ...
            response = requests.get(
                f"{self.base_url}/api/pub/v2/services",
                headers=headers,
                timeout=10
            )
            response.raise_for_status()
    ...
    async def create_verification_enhanced(self, request: VerificationRequest) -> Tuple[str, Dict]:
        ...
        try:
            ...
            if e.response and e.response.status_code == 401:
                ...
                headers = {"Authorization": f"Bearer {await self.get_token(force_refresh=True)}"}
                response = requests.post(
                    f"{self.base_url}/api/pub/v2/verifications",
                    headers=headers,
                    json=payload,
    ...
    async def get_verification_details(self, verification_id: str) -> Dict:
        ...
        try:
            ...
            return {
                "id": verification_id,
                "status": status_map.get(data.get("state"), VerificationStatus.PENDING).value,
        ...
        try:
            ...
            for sms in data.get("data", []):
                messages.append({
                    ...
                    "verification_code": self._extract_verification_code(sms.get("smsContent", ""))
                })
    ...
    async def bulk_create_verifications(self, requests_list: List[VerificationRequest]) -> List[Tuple[str, Dict]]:
        ...
        for i in range(0, len(requests_list), batch_size):
            ...
            tasks = [
                self.create_verification_enhanced(req) for req in batch
            ]
    ...
    async def get_account_balance(self) -> Dict:
        ...
        try:
            headers = {"Authorization": f"Bearer {await self.get_token()}"}
            response = requests.get(
                f"{self.base_url}/api/pub/v2/account/me",
                headers=headers,
                timeout=10
            )
            response.raise_for_status()
...
async def optimize_service_selection(client: EnhancedTextVerifiedClient, 
                                   preferred_services: List[str]) -> str:
...
async def smart_carrier_selection(client: EnhancedTextVerifiedClient,
                                service_name: str,
                                user_preferences: Dict) -> CarrierFilter:
