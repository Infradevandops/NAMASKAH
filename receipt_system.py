...
class InAppNotification:
    ...
    def __init__(
        self, user_id, title, message, notification_type="receipt", verification_id=None
    ):
        self.user_id = user_id
        self.title = title
...
class NotificationService:
    ...
    def create_notification(
        self, user_id, title, message, notification_type="info", verification_id=None
    ):
    ...
    def get_notification_preferences(self, user_id):
        ...
        prefs = self.db.query(PrefsModel).filter(PrefsModel.user_id == user_id).first()
    ...
    def update_notification_preferences(self, user_id, **kwargs):
        ...
        if not prefs:
            prefs = PrefsModel(
                id=f"prefs_{datetime.now(timezone.utc).timestamp()}", user_id=user_id
            )
            self.db.add(prefs)
