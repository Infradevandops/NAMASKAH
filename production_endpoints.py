...
def production_health_check(admin: User = Depends(get_admin_user)):
    ...
    recent_errors = len(
        [
            ...
            if e["timestamp"] > (datetime.now(timezone.utc).timestamp() - 3600) * 1000
        ]
    )
    error_rate_ok = recent_errors < 10  # Less than 10 errors per hour

    overall_health = (
        "healthy" if all([db_healthy, tv_healthy, error_rate_ok]) else "degraded"
    )
...
@app.post(
    "/admin/production/clear-logs", tags=["Admin"], summary="Clear Production Logs"
)
