...
from textverified_optimization import (
    EnhancedTextVerifiedClient,
    VerificationRequest,
    CarrierFilter,
)
...
class BulkVerificationRequest(BaseModel):
    ...
    batch_webhook_url: Optional[str] = Field(
        None, description="Webhook for batch completion"
    )
...
async def create_smart_verification(
    ...
    tv_client = EnhancedTextVerifiedClient(
        api_key=os.getenv("TEXTVERIFIED_API_KEY"), email=os.getenv("TEXTVERIFIED_EMAIL")
    )

    try:
        verification_id, verification_data = (
            await tv_client.create_verification_enhanced(verification_request)
        )
        ...
        background_tasks.add_task(
            monitor_verification_progress, verification_id, user.id, db
        )
        ...
        raise HTTPException(
            status_code=503, detail=f"Verification creation failed: {str(e)}"
        )
...
async def create_bulk_verifications(
    ...
    if len(request.verifications) > 10:
        raise HTTPException(
            status_code=400, detail="Maximum 10 verifications per batch"
        )
    ...
    results = await tv_client.bulk_create_verifications(
        [vr[1] for vr in verification_requests]
    )
    ...
    for i, (result, (original_req, verification_req, cost)) in enumerate(
        zip(results, verification_requests)
    ):
...
async def get_pricing_analysis(
    ...
    # Get user context
    monthly_count = (
        get_user_monthly_count(user.id, db)
        if request.monthly_usage == 0
        else request.monthly_usage
    )
    ...
    if request.include_forecast:
        response.forecast = pricing_engine.get_pricing_forecast(
            request.service_name, 24
        )
...
async def get_service_recommendations(
    ...
    user_history = (
        db.query(Verification.service_name)
...
async def optimize_existing_verification(
    verification_id: str, user=Depends(get_current_user), db: Session = Depends(get_db)
):
    """
    ...
    if verification.status != "pending":
        raise HTTPException(
            status_code=400, detail="Can only optimize pending verifications"
        )
    ...
    # Get alternatives
    alternatives = optimizer.recommend_service_alternatives(
        verification.service_name, 3
    )
    ...
    return {
        ...
        "recommendation": (
            "Consider switching to an alternative service"
            if alternatives
            else "No better alternatives available"
        ),
    }
...
async def export_analytics_report(
    format: str = "json", admin=Depends(get_admin_user), db: Session = Depends(get_db)
):
    """
...
async def monitor_verification_progress(
    verification_id: str, user_id: str, db: Session
):
...
async def monitor_batch_progress(
    batch_id: str, verification_ids: List[str], user_id: str, webhook_url: Optional[str]
):
...
def get_user_monthly_count(user_id: str, db: Session) -> int:
    ...
    return (
        db.query(Verification)
        .filter(Verification.user_id == user_id, Verification.created_at >= month_start)
        .count()
    )
...
def get_user_analytics(user_id: str, db: Session) -> Dict:
    ...
    success_rate = (
        (successful / total_verifications * 100) if total_verifications > 0 else 0
    )
    ...
    return {
        ...
        "avg_cost_per_verification": (
            round(total_spent / total_verifications, 2)
