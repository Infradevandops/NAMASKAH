...
class ProductionTester:
    ...
    def login_admin(self):
        ...
        login_data = {"email": "admin@namaskah.app", "password": "Namaskah@Admin2024"}
    ...
    def test_caching(self):
        ...
        ), "Services endpoint should work on second call"

        # Second call should be faster (cached)
        if second_call_time < first_call_time * 0.8:
            print(
                f"   ✓ Caching working (first: {first_call_time:.3f}s, second: {second_call_time:.3f}s)"
            )
        else:
            print(
                f"   ⚠️ Caching may not be working (first: {first_call_time:.3f}s, second: {second_call_time:.3f}s)"
            )
    ...
    def test_health_checks(self):
        ...
        print(
            f"   ✓ TextVerified API: {health_data.get('textverified_api', 'unknown')}"
        )
    ...
    def test_dashboard_performance(self):
        ...
        assert (
            "performance-monitor.js" in content
