...
async def report_error_rate(request: Request):
    ...
    try:
        ...
        if error_rate > 5:
            logger.warning(
                f"High error rate reported: {error_rate}% - {data.get('error_count', 0)} errors"
            )
...
@app.post(
    "/api/v2/verify/smart", tags=["Verification"], summary="Smart SMS Verification"
)
def create_smart_verification(
    ...
    if OPTIMIZATIONS_AVAILABLE:
        try:
            ...
            carrier_filter = CarrierFilter(
                carrier=req.carrier, area_code=req.area_code, exclude_voip=True
            )
    ...
    try:
        verification_id = tv_client.create_verification(
            req.service_name, req.capability
        )
...
def get_receipt_stats(
    ...
    email_enabled = (
        ...
        .filter(NotificationPreferences.email_notifications is True)
        .count()
    )
    receipts_enabled = (
        db.query(NotificationPreferences)
        .filter(NotificationPreferences.receipt_notifications is True)
        .count()
    )
...
async def send_webhook(user_id: str, verification_id: str, messages: list, db: Session):
    ...
    webhooks = (
        db.query(Webhook)
        .filter(Webhook.user_id == user_id, Webhook.is_active is True)
        .all()
    )
    ...
    if OPTIMIZATIONS_AVAILABLE:
        try:
            ...
            if user.is_admin:
                ...
                user_verifications = (
                    db.query(Verification).filter(Verification.user_id == user.id).all()
                )
                total_verifications = len(user_verifications)
                successful = len(
                    [v for v in user_verifications if v.status == "completed"]
                )
                success_rate = (
                    (successful / total_verifications * 100)
                    if total_verifications > 0
                    else 0
                )
