...
class ServiceOptimizer:
    ...
    def get_service_metrics(self, service_name: str) -> ServiceMetrics:
        ...
        # Check cache
        if (
            cache_key in self.service_cache
            and cache_key in self.cache_expiry
            and datetime.now(timezone.utc) < self.cache_expiry[cache_key]
        ):
        ...
        if not verifications:
            ...
            success_rate = (
                (len(successful) / total_attempts) * 100 if total_attempts > 0 else 0
            )
            ...
            avg_delivery_time = (
                sum(delivery_times) / len(delivery_times) if delivery_times else 30.0
            )
            ...
            recent_failures = len(
                [
                    ...
                    if v.created_at >= seven_days_ago
                    and v.status in ["cancelled", "failed"]
                ]
            )
    ...
    def recommend_service_alternatives(
        self, service_name: str, count: int = 3
    ) -> List[Dict]:
    ...
    def detect_service_issues(self, service_name: str) -> List[Dict]:
        ...
        # No recent successes
        if metrics.last_success and metrics.last_success < datetime.now(
            timezone.utc
        ) - timedelta(days=7):
            issues.append(
                {
    ...
    def get_service_health_report(self) -> Dict:
        ...
        report["needs_attention"] = [
            s for s in all_services if s["health_status"] in ["degraded", "failed"]
        ]
...
class SmartServiceRouter:
    ...
    async def route_verification_request(
        self, preferred_service: str, user_preferences: Dict, fallback_count: int = 3
    ) -> Tuple[str, Dict]:
        ...
        alternatives = self.optimizer.recommend_service_alternatives(
            preferred_service, fallback_count
        )
...
def get_service_recommendations(db: Session, user_history: List[str]) -> List[Dict]:
    ...
    for service, usage_count in sorted(
        ...
        for alt in alternatives:
            if (
                alt["success_rate"]
                > optimizer.get_service_metrics(service).success_rate
            ):
...
async def optimize_bulk_verifications(
    db: Session, service_requests: List[str]
) -> List[Tuple[str, str]]:
