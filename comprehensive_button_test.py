...
class ComprehensiveButtonTester:
    ...
    def test_button(self, button_name, endpoint, method="GET", data=None, expected_status=200):
    ...
    def test_all_buttons(self):
        ...
        self.test_button("Fund Wallet (Paystack)", "/wallet/paystack/initialize", "POST", wallet_data, 200)
    ...
    def test_button_interactions(self):
        ...
        self.test_button("Create API Key", "/api-keys/create", "POST", api_key_data, 200)
        
        # Test Webhook Creation
        webhook_data = {"url": "https://example.com/webhook"}
        self.test_button("Create Webhook", "/webhooks/create", "POST", webhook_data, 200)
        
        # Test Verification Creation (might fail due to credits/config)
        verification_data = {"service_name": "telegram", "capability": "sms"}
        self.test_button("Create Verification", "/verify/create", "POST", verification_data, 200)
    ...
    def generate_report(self):
        ...
        success_rate = (len(working_buttons) / total_buttons * 100) if total_buttons > 0 else 0
        ...
        if failed_buttons:
            ...
            for button in failed_buttons:
                result = self.button_results[button]
                self.log(f"   • {button}: {result['endpoint']} ({result.get('response_code', 'N/A')})", "ERROR")
        
        if error_buttons:
            self.log(f"\n🔧 ERROR BUTTONS ({len(error_buttons)}):", "ERROR")
            for button in error_buttons:
                result = self.button_results[button]
                self.log(f"   • {button}: {result.get('error', 'Unknown error')}", "ERROR")
        
        # Productivity Assessment
        self.log(f"\n🎯 PRODUCTIVITY ASSESSMENT:", "INFO")
        if success_rate >= 90:
            self.log("   🎉 EXCELLENT: All buttons are highly functional and productive", "SUCCESS")
        elif success_rate >= 75:
            self.log("   ✅ GOOD: Most buttons are working well with minor issues", "SUCCESS")
        elif success_rate >= 50:
            self.log("   ⚠️ MODERATE: Some buttons need attention for full productivity", "ERROR")
        else:
            self.log("   ❌ POOR: Many buttons need fixing for proper functionality", "ERROR")
        
        # Recommendations
        self.log(f"\n💡 RECOMMENDATIONS:", "INFO")
        if failed_buttons:
            self.log("   1. Fix failed button endpoints for complete functionality", "INFO")
        if error_buttons:
            self.log("   2. Debug error buttons to improve reliability", "INFO")
        if success_rate < 100:
            self.log("   3. Test button interactions in browser for user experience", "INFO")
        self.log("   4. Monitor button performance in production environment", "INFO")
        
        return success_rate, len(working_buttons), len(failed_buttons), len(error_buttons)
